...
class _HistoryScreenState extends State<HistoryScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadSensorHistory() async {
    ...
    try {
      final history = await HybridDataService().getSensorHistory(_selectedPlantId!, _selectedDays);
      setState(() {
                        ...
                        return DropdownMenuItem<String>(
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                radius: 12,
                                backgroundColor: _getHealthColor(plant.healthStatus),
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Chart placeholders
              _buildChartCard('Soil Moisture Trends', Icons.water_drop, AppColors.info),
              const SizedBox(height: 16),
              _buildChartCard('Temperature History', Icons.thermostat, Colors.orange),
              const SizedBox(height: 16),
              _buildChartCard('Light Level Changes', Icons.wb_sunny, Colors.amber),
  ...
  Widget _buildLogsTab() {
    return ListView(
      ...
      children: [
        ...
        _buildLogEntry('Light adjusted', 'Grow light intensity increased to 80%', DateTime.now().subtract(const Duration(hours: 4)), Icons.wb_sunny, Colors.orange),
        _buildLogEntry('Health check', 'Plant health assessment: Good condition', DateTime.now().subtract(const Duration(hours: 8)), Icons.health_and_safety, AppColors.success),
        _buildLogEntry('Moisture alert', 'Low soil moisture detected - 25%', DateTime.now().subtract(const Duration(hours: 12)), Icons.warning, AppColors.warning),
      ...
      builder: (context, plantProvider, child) {
        ...
        return ListView(
          ...
          children: [
            _buildInsightCard(
              'Health Score',
              selectedPlant?.isHealthy == true ? 'Excellent' : 'Needs Attention',
              ...
              selectedPlant?.isHealthy == true ? AppColors.success : AppColors.warning,
            ),
